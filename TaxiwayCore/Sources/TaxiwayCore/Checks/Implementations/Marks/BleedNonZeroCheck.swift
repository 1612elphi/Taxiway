...
public struct BleedNonZeroCheck: ParameterisedCheck {
    public static let typeID = "marks.bleed_nonzero"
    public let id: UUID
    public let parameters: EmptyParameters
    public let severityOverride: CheckSeverity?

    public var name: String { "Bleed is Non-Zero" }
    ...
    public func run(on document: TaxiwayDocument) -> CheckResult {
        ...
        if affected.isEmpty {
            return pass(message: "No pages have bleed")
        }

        return fail(
            message: "\(affected.count) page(s) have non-zero bleed",
